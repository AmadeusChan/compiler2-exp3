...
//
  ...
  void visitBasicBlock(BasicBlock &B) {
	  ...
	  for (auto i = B.begin(), j = B.end(); i != j; ++ i) {
		  ...
		  if (isa<BinaryOperator>(&inst)) {
			  ...
			  visitCallInst(*dyn_cast<CallInst>(&inst));
		  } else if (isa<ReturnInst>(&inst)) {
			  visitReturnInst(*dyn_cast<ReturnInst>(&inst));
		  }
	  }
  }
  ...
  z3::expr getPreCondition() {
	  ...
	  if (current_bb_name == getName(current_function->getEntryBlock())) {
  ...
  void visitPHINode(PHINode &I) {
	  ...
	  for (unsigned i = 0; i < num_incoming_edges; ++ i) {
	          ...
	          for (auto p: vec_expr) {
				  ...
				  //solver.add(forall(getFunctionArgsVarVec(), (!p.second || dst_expr == value_expr)));
